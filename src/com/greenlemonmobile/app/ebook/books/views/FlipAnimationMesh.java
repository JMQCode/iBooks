 ...
 * @author harism
 */
public class FlipAnimationMesh {
	...
	private final FlipAnimationPage mTexturePage = new FlipAnimationPage();
	 ...
	 */
	public FlipAnimationMesh(int maxCurlSplits) {
		...
		for (int i = 0; i < mArrScanLines.size(); ++i) {
			...
			while (mArrOutputVertices.size() > 0) {
				...
				if (textureFront != mFlipTexture) {
					...
					v.mColor = mTexturePage.getColor(FlipAnimationPage.SIDE_FRONT);
				} else {
					v.mTexX *= mTextureRectBack.right;
					v.mTexY *= mTextureRectBack.bottom;
					v.mColor = mTexturePage.getColor(FlipAnimationPage.SIDE_BACK);
				}
	 ...
	 */
	public synchronized FlipAnimationPage getTexturePage() {
		return mTexturePage;
	}
	...
	public synchronized void reset() {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (mFlipTexture) {
				...
				tmp.mColor = mTexturePage.getColor(FlipAnimationPage.SIDE_BACK);
			} else {
				tmp.mTexX *= mTextureRectFront.right;
				tmp.mTexY *= mTextureRectFront.bottom;
				tmp.mColor = mTexturePage.getColor(FlipAnimationPage.SIDE_FRONT);
			}
